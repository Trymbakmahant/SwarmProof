...
/** Build the five specialists with identities (payment addresses configurable via env). */
export function createSpecialistAgents(
  addresses: Record<SpecialistId, string> = SPECIALIST_PAYMENT_ADDRESSES,
...
export async function runSpecialists(
  agents: Record<SpecialistId, SecurityAgent>,
  task: SecurityTask,
): Promise<Array<{ agentId: string; findings: Finding[] }>> {
  const entries = SPECIALIST_IDS.map((agentId) =>
    agents[agentId].analyze(task).then((findings) => ({ agentId, findings })),
  );
